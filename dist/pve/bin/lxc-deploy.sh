...
lxc_deploy() {
  ...
  print_usage() { echo "
   ...
   ,  [--disk DISK] [--ram RAM] [--cores CORES] [--privileged] [--onboot] \\
   ,  [--pass PASS] [--pass-envvar PASS_ENVVAR='$(lh_params default-string PASS_ENVVAR)'] \\
  ...
  print_help() { text_nice "
    Deploy LXC container using self-contained script. Supported LXC-containers:
  ...
  _int_after_create_vaapi() {
    ...
    "$(lh_params get PRIVILEGED)" || [[ -e "/dev/dri/renderD128" ]] || return 0
