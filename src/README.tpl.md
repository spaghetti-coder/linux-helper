...
<details><summary>pve/bin/lxc-deploy.sh</summary>
  ...
  <!-- .LH_ADHOC_USAGE:pve/bin/lxc-deploy.sh -->
  ...
  (
    ...
    set -x; "${dl_tool[@]}" "@@BASE_RAW_URL/${LH_VERSION:-master}/dist/pve/bin/lxc-deploy.sh" \
    || "${dl_tool[@]}" "@@BASE_RAW_URL_ALT/${LH_VERSION:-master}/dist/pve/bin/lxc-deploy.sh"
  ) | (DEST=./my-lxc.sh; set -x; tee -- "${DEST}" >/dev/null; chmod +x -- "${DEST}")
  ~~~
  <!-- .LH_HELP:pve/bin/lxc-deploy.sh -->
</details>  
